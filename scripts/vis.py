...
ap_folders = [f for f in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, f))]

for ap in ap_folders:
    ap_path = os.path.join(data_dir, ap)
    ap_out_path = os.path.join(output_dir, ap)
    os.makedirs(ap_out_path, exist_ok=True)
    ...
    while current_time < end_time:
        ...
        if has_data:
            ...
            filename = f"AP_{folder_number}_{current_time.strftime('%Y%m%d_%H%M')}_to_{window_end.strftime('%H%M')}.png"
            plt.savefig(os.path.join(ap_out_path, filename))
